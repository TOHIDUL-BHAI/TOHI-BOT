...
module.exports.run = async function({ api, event, Users }) {
    try {
        ...
        if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
            try {
                // Auto set bot nickname (always enabled)
                try {
                    const autoSetNameModule = require('../commands/autosetname.js');
                    const result = await autoSetNameModule.setBotNickname(api, threadID);
                    
                    if (result.success) {
                        console.log(`✅ Auto nickname set in group ${threadID}: ${result.nickname}`);
                    } else {
                        console.log(`⚠️ Auto nickname failed in group ${threadID}: ${result.error}`);
                        // Fallback to manual nickname setting
                    ...
                    console.error('Auto set name error:', autoError);
                    // Fallback to manual nickname setting
                    const botname = global.config.BOTNAME || "TOHI-BOT";
                    await api.changeNickname(`[ ${global.config.PREFIX} ] • ${botname}`, threadID, api.getCurrentUserID());
                }
